...
/// Manage the payments of one [OnUsDebt] (a debt the business owes to
/// someone else). Mirrors [DebtPaymentsDialog] for customer debts.
class OnUsPaymentsDialog extends StatefulWidget {
  final OnUsDebt debt;
  const OnUsPaymentsDialog({required this.debt, super.key});

  @override
  State<OnUsPaymentsDialog> createState() => _OnUsPaymentsDialogState();
}

class _OnUsPaymentsDialogState extends State<OnUsPaymentsDialog> {
  ...
  void initState() {
    ...
    context.read<OnUsDebtsCubit>().openDebt(widget.debt.id);
  }
  ...
  List<Map<String, String>> _rows(List<OnUsPayment> ps) => ps
      .map(
        (p) => {
          'id': p.id.toString(),
  ...
  void _editSelected(OnUsDebtsState state) {
  ...
  void _deleteSelected(OnUsDebtsState state) {
    final sel = tableController.selectedIndexes;
    if (sel.isEmpty) {
    ...
    return Directionality(
      ...
      child: Dialog(
        ...
        child: Container(
          ...
          child: BlocBuilder<OnUsDebtsCubit, OnUsDebtsState>(
            builder: (context, state) {
              final total = state.totalOf(widget.debt);
              final remaining = total - state.openPaid; // may be negative
              return Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text(
                        'دفعات: ${widget.debt.name} (وصل ${widget.debt.bill ?? ''})',

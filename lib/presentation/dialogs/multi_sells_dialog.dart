...
/// Dialog to manage the furniture line-items (multiSells) of a sale.
///
/// Reads/writes the draft list held in [SellsCubit]. The price is resolved
/// automatically from the chosen model and sell type.
class MultiSellsDialog extends StatefulWidget {
  final String bill;

  const MultiSellsDialog({required this.bill, super.key});

  @override
  State<MultiSellsDialog> createState() => _MultiSellsDialogState();
}

class _MultiSellsDialogState extends State<MultiSellsDialog> {
  final TextEditingController colorController = TextEditingController();
  final TextEditingController countController = TextEditingController();
  final SHTableController tableController = SHTableController();
  final SellsModelsRepository _modelsRepo = SellsModelsRepository();

  SellsModel? selectedModel;
  SellType selectedType = SellType.set;
  int? setNumber; // sets count (set type)
  int? chairCount; // 7/8/10 (set type)
  ...
  SellsModel? _staleModelOption;

  /// Bumped whenever the form is reset or loaded for editing. Used as part of
  /// the dropdown keys so the fields rebuild fresh and re-read [initialValue]
  /// (DropdownButtonFormField.initialValue is initial-only and won't reflect
  /// a programmatic change to a bound value otherwise).
  int _formVersion = 0;
  ...
  List<Map<String, String>> _toRows(List<MultiSell> items) {
    return items
        .map(
          (m) => {
            'model': m.modelName,
  ...
  /// Normalize the color field: empty -> null.
  String? get _colorOrNull {
    final c = colorController.text.trim();
    return c.isEmpty ? null : c;
  }

  /// Build a MultiSell from the current form inputs.
  MultiSell? _buildFromForm() {
    ...
    if (selectedType == SellType.set) {
      ...
      return MultiSell.fromForm(
        id: 0,
        bill: widget.bill,
        model: selectedModel!,
        type: SellType.set,
        setNumber: setNumber!,
        count: chairCount!.toDouble(), // 7 / 8 / 10
        color: _colorOrNull,
      );
    }
  ...
  void _clearForm() {
    ...
    _formVersion++; // force dropdowns to rebuild blank
    if (mounted) setState(() {});
  }
  ...
  Future<void> _editSelected() async {
    ...
    final items = context.read<SellsCubit>().state.draftMultiSells;
    final idx = selected.first;
    if (idx >= items.length) return;

    final m = items[idx];

    final activeModels = context.read<SellsModelsCubit>().state.models;
    SellsModel? match;
    ...
    setState(() {
      ...
      if (m.type == SellType.set) {
        ...
        // not a set: clear any stale set-only state
        setNumber = null;
        chairCount = null;
      ...
      _formVersion++; // force dropdowns to reflect the loaded values
    });
  ...
  void _deleteSelected() {
    ...
    final cubit = context.read<SellsCubit>();
    // remove from highest index down so positions stay valid
  ...
  Widget _buildInputRow() {
    return BlocBuilder<SellsModelsCubit, SellsModelsState>(
            ...
            Row(
              ...
              children: [
                ...
                Expanded(
                  ...
                  child: _Labeled(
                    label: 'الموديل',
                    child: DropdownButtonFormField<SellsModel>(

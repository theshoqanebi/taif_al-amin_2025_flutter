...
/// Real schema (OnUsPayments):
///   id, debt_id INTEGER, payment_amount INTEGER, payment_date TEXT, notes TEXT
class OnUsPaymentsRepository {
  static const _table = 'OnUsPayments';

  Future<List<OnUsPayment>> getByDebtId(int debtId) async {
    final results = await DatabaseService.query(
      'SELECT * FROM $_table WHERE debt_id = ? ORDER BY payment_date ASC, id ASC',
      [debtId],
    );
    return results.map(OnUsPayment.fromMap).toList();
  }
  ...
  Future<int> insert(OnUsPayment payment) async {
  ...
  Future<int> update(OnUsPayment payment) async {
    return DatabaseService.execute(
